...
import { create as createPackage, createComponent as createComponentPackage } from '@workspacein8/package';
...
export default async function create(opts) {
  ...
  if (fs.existsSync(opts.out_dir)) {
    console.error(
      `\n${chalk.red('Error:')} ${chalk.italic.yellow(
        opts.out_dir
      )} already exists. We don't want to override any important files so either rename the workspace or delete the folder to progress`
    );
    return;
  }
  ...
  if (opts.debug) {
    console.debug(
      `${chalk.blue('Your options: ')}${chalk.blue(
  ...
  await createWorkspace(opts);
  await createPackageDir(opts);
 ...
 */
function initDirs(opts) {
  fs.mkdirSync(`${opts.out_dir}`);
  if (!fs.existsSync(`${opts.out_dir}/packages`)) {
    fs.mkdirSync(`${opts.out_dir}/packages`);
  }
  if (!fs.existsSync(`${opts.out_dir}/packages/${opts.package_name}`)) {
    fs.mkdirSync(`${opts.out_dir}/packages/${opts.package_name}`);
  }
}

function initGit(opts) {
  execSync('git init', { cwd: opts.out_dir});
  execSync('git checkout -b main', { cwd: opts.out_dir});
}
 ...
 * @param {CreateOpts} opts
 */
async function createPackageDir(opts) {
  if (opts.template_type === 'components') {
    await createComponentPackage({
      ...opts,
      out_dir: `${opts.out_dir}/packages/${opts.package_name}`,
    });
  } else {
    await createPackage({
      ...opts,
...
function addDeps(opts) {
  ...
  let out = PackageManager.run(opts.workspace_type).addDev(
    'eslint vite jsdoc mocha chai'
  );
...
function addTypeDecleration(opts) {
  try {
    execSync(
      `npx npx -p typescript tsc ${opts.out_dir}/packages/${opts.package_name}/index.js --declaration --emitDeclarationOnly --allowJs`,
      { cwd: opts.out_dir }
    );
...
function addExample(opts) {
  ...
  const out = PackageManager.run(opts.workspace_type).init(
    'vite example',
    '--template vanilla'
  );
