...
export class PackageManager {
  ...
  static run(manager) {
    if (
      ...
      PackageManager.instance === undefined
    ) {
  ...
  constructor(cwd, manager, exec = execSync) {
    ...
    this.exec = (command) => {
      if (PackageManager.debug)
        console.debug(`${chalk.yellow('running:')} ${command}`);
      try {
        ...
        console.error(chalk.red(` --- --- ---`));
        throw new Error(
          `Executing ${manager} command "${command}" failed at ${process.env.PWD}/${this.cwd} - ${error.message}`
        );
      }
    };
    this.install();
